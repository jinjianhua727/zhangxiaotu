...
var App = (function () {
    ...
    if (document.addEventListener) {
        document.addEventListener("DOMContentLoaded", function () {
            ...
            createPersonalMessage.addEventListener("click", function (evt) {

                if (editorVisible) hidePersonalMessageEditor();
                else showPersonalMessageEditor();

            }, true);

            messageTextEditor.addEventListener("input", function (evt) {
                var message = messageTextEditor.value;
                Postcard.setPersonalizedMessage(message);
            }, true);

            messageTextEditor.addEventListener("keydown", function (evt) {
                enteringText = true;
            }, true);

            messageTextEditor.addEventListener("keyup", function (evt) {
                enteringText = false;
            }, true);

            var shareButtons = document.querySelectorAll("a.shareButton");
            for (var ii = 0; ii < shareButtons.length; ii++) {

                if (shareButtons[ii].id === "copyPersonalURL" &&
                    !window.clipboardData) {
                    // hide for browsers not supporting window.clipboardData
                    shareButtons[ii].className = "hidden";
                }

                shareButtons[ii].addEventListener("click", function (evt) {
                    window.location.hash = escape(messageTextEditor.value);
                    if (window.clipboardData && clipboardData.setData) {
                        clipboardData.setData("text", window.location.href);
                        setTimeout(hidePersonalMessageEditor, 500);
                    } 

                    if (evt.target &&
                    evt.target.id === "shareFacebook" &&
                    evt.target.href) {
                        evt.target.href = evt.target.href.replace("title", document.title);
                        evt.target.href = evt.target.href.replace("link", escape(window.location));
                    }
                });
            }
    ...
    function showPersonalMessageEditor() {
        personalMessageEditor.style.right = "-1px";
        editorVisible = true;
    }

    function hidePersonalMessageEditor() {
        personalMessageEditor.style.right = "-425px";
        editorVisible = false;
    }
